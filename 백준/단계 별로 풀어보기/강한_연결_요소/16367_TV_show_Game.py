...
def solve():
    ...
    for i in range(M):
        IN = input().split()
        edited = [int(IN[j - 1]) * (1 if IN[j] == 'R' else -1) for j in range(1, 6, 2)]
        edge[-edited[0]].add(edited[1])
        edge[-edited[0]].add(edited[2])
        edge[-edited[1]].add(edited[0])
        edge[-edited[1]].add(edited[2])
        edge[-edited[2]].add(edited[0])
        edge[-edited[2]].add(edited[1])
    ...
    for i in range(1, N + 1):
        if element_locate[i] < element_locate[-i]: result += ["R"]
        elif element_locate[i] == element_locate[-i]:
            isit = False
            break
        else: result += ["B"]
    
    print(''.join(result) if isit else '-1')
